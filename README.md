...
- **AI-Augmented CTI:** Прожеіть кожен заінжещений IOC через Claude із промптом на кшталт: *«Для цього IOC (IP / domen / hash) опиши одним реченням найімовірнішу тактику та техніку (MITRE ATT&CK).»* Збережіть LLM-опис як `data.llm_tag`; показуйте його у відповідях `/enrich`.
- **TTP / AI attack patterns:** Коли тягнете URLhaus / ThreatFox, додайте фільтр тегів на записи, що посилаються на `phishing-kit`, `generated-phishing` або кампанії, які фіди позначають як автоматизовані. Виставте endpoint `/ioc/ai-campaigns`, що перераховує саме ці.
- **AI social engineering:** Включіть щонайменше один фід, що стосується AI-generated phishing-доменів. `ThreatFox` уже це тегує; виведіть їх в окрему панель дашборду.
